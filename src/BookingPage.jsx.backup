...
const WORK_START = 9 * 60, WORK_END = 17 * 60; // journée de travail 09:00–17:00
const HEURE_DURATION = 60; // durée par défaut d'un cours "Heure"

const busyIntervals = (dateKey, reservations) =>
  reservations.filter(r => r.date === dateKey && r.statut !== 'Annulée').map(r => [timeToMinutes(r.heureDebut), timeToMinutes(r.heureFin)]);

const overlaps = (start, end, intervals) => intervals.some(([s, e]) => start < e && end > s);

const availableHourStarts = (dateKey, reservations) => {
  const busy = busyIntervals(dateKey, reservations);
  const starts = [];
  for (let m = WORK_START; m + HEURE_DURATION <= WORK_END; m += 30) {
    if (!overlaps(m, m + HEURE_DURATION, busy)) starts.push(m);
  }
  return starts;
};
const isCreneauFree = (dateKey, creneau, reservations) => {
  const busy = busyIntervals(dateKey, reservations);
  const [s, e] = CRENEAUX[creneau].map(timeToMinutes);
  return !overlaps(s, e, busy);
};
const isJourneeFree = (dateKey, reservations) => {
  const busy = busyIntervals(dateKey, reservations);
...
async function loadReservations() {
  try { const r = await window.storage.get(RES_KEY); return JSON.parse(r.value); } catch (e) { return []; }
}
...
async function loadSettings() {
  try { const r = await window.storage.get(SETTINGS_KEY); return { ...DEFAULT_SETTINGS, ...JSON.parse(r.value) }; } catch (e) { return DEFAULT_SETTINGS; }
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => { setSettings(await loadSettings()); setReservations(await loadReservations()); })();
  }, []);
  ...
  const hourOptions = useMemo(() => availableHourStarts(form.date, reservations), [form.date, reservations]);
  const matinFree = useMemo(() => isCreneauFree(form.date, 'Matin', reservations), [form.date, reservations]);
  const apremFree = useMemo(() => isCreneauFree(form.date, 'Après-midi', reservations), [form.date, reservations]);
  ...
  const handleSubmit = async () => {
    ...
    const heureDebut = form.heureDebut || '09:00';
    const heureFin = form.heureFin || '10:00';
    const current = await loadReservations();
    const busy = busyIntervals(form.date, current);
    if (overlaps(timeToMinutes(heureDebut), timeToMinutes(heureFin), busy)) {
      setLoading(false);
    ...
    const reservation = {
      id: Date.now(), nom: form.nom, prenom: form.prenom, telephone: form.telephone, email: form.email,
      ...
      prix: estimate, statut: 'En attente', paiement: 'Non payé', modePaiement: 'Non renseigné',
    ...
    await persistReservations([...current, reservation]);
